    ...
    <div className="container mb-5">
      <div className="d-flex flex-row align-items-start">
          ...
          {items.map(item => <div className="cart-item p-3">
            <div className="d-flex flex-row">
              ...
              <div className="col-2 p-2">
                Quatity
                <select  value={item.quantity} onChange={(e) => changeQuantity(e.target.value, item)}>
        ...
        {order.items.length ? <div className="col-4 order p-3 m-2">
          ...
          <div className="d-flex flex-row justify-content-between p-2">
            <span className="billing-item">Discount ({order.discontIn_percent}%)</span>
            <span className="billing-cost">-${order.total_cost * order.discontIn_percent / 100}</span>
          </div>
          <div className="d-flex flex-row justify-content-between p-2">
            <span className="billing-item fs-5">Total</span>
            <span className="billing-cost fs-5">${order.total_cost - order.total_cost * order.discontIn_percent / 100 + order.shipping_Charge}</span>
          </div>
